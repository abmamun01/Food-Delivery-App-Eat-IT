...
public class MyCartAdapter extends RecyclerView.Adapter<MyCartAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        Glide.with(context)
                .load(cartItemList.get(position).getFoodImage()).into(holder.img_cart);
        ...
        //Event
        holder.numberButton.setNumber(String.valueOf(cartItemList.get(position).getFoodQuantitiy()));
        holder.numberButton.setOnValueChangeListener((view, oldValue, newValue) -> {

            //When User Click this button , we will update database
            cartItemList.get(position).setFoodQuantitiy(newValue);
